...
%Conversão da anomalia media para verdadeira
...
%Condição inicial do item A
InitCond = [r_a v_a];

%Condição inicial do item B
InitCond2 = [r_b v_b];

%Solução para o item a |(Possível modificar o multiplicar em T_a e T_b)
[Times,Out] = ode45(@edos, [0 1*T_a], InitCond);

%Solução para o item b
...
%Solução Analítica (Geração dos pontos em vermelho)
